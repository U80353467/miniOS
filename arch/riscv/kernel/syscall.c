...
struct ret_info syscall(uint64_t syscall_num, uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5, uint64_t sp) {
    ...
    case SYS_FORK: {
        ...
        list_for_each_entry(vma, &current->mm.vm->vm_list, vm_list) {
            struct vm_area_struct * copy_vma = kmalloc(sizeof(struct vm_area_struct));
            memcpy(copy_vma, vma, sizeof(struct vm_area_struct));
            list_add(&(copy_vma->vm_list), &(task[i]->mm.vm->vm_list));
            if (vma->mapped) {
                uint64_t physical_page = alloc_pages((vma->vm_end - vma->vm_start) / PAGE_SIZE);
                create_mapping((uint64_t *)root_page_table, vma->vm_start, physical_page, vma->vm_end - vma->vm_start, vma->vm_flags);
                uint64_t pte = get_pte((current->satp & ((1ULL << 44) - 1)) << 12, vma->vm_start);
                memcpy((uint64_t *)physical_page, (uint64_t *)((pte >> 10) << 12), vma->vm_end - vma->vm_start);
            }
        }
        ...
        uint64_t * task_sp_ptr = (uint64_t)sp_ptr + (uint64_t)task[i] - (uint64_t)current;
        task_sp_ptr[4] = 0;
    ...
    case SYS_WAIT: {
        ...
        while (is_wait) {
            ...
            for (int i = 0; i < NR_TASKS; i++) {
                if (task[i] != NULL && task[i]->pid == arg0) {
                    if (task[i]->counter > 0) {
                        is_wait = 1;
                        ...
                        uint64_t task_pa = (get_pte(root_page_table, task[i]) >> 10) << 12;
